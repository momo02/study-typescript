...
// 인터페이스는 이름은 대문자로 시작.
// 작성한 인터페이스는 string 이나 number 처럼 타입으로써 사용 가능.
interface Student {
  // Property
  readonly studentID: number;
  studentName: string;
  age?: number; // ?를 붙이면 필수가 아닌 Optional(선택적) Property
  gender: string;
  subject: string;
  courseCompleted: boolean;
  // Method : 인터페이스 내에서 선언된 함수
  // 표현방법만 다른 같은 method
  //addComment(comment: string): string;
  addCommnet?: (comment: string) => string; //(arrow function 사용)
}
